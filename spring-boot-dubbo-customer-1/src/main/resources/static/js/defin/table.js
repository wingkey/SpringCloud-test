...
var pageNum=1;// 第几页
        ...
        function InitMainTable () {
            ...
            var queryUrl = '../testdubbo/getUser'
                ...
                columns: [{
                    ...
                    formatter: function (value, row, index){ // 单元格格式化函数
                    	...
                    	if (value == 0) {
                    		...
                    		text = '<span style="color:red">'+"注销"+'</span>'; 
                    		} 
                    	return text; 
                    }    
   ...
   function insertUser(){
	   ...
	   var sex=$('#modal-sex1').selectpicker('val');
	   var state=$('#modal-state1').selectpicker('val');
	   ...
	   if (bv1.isValid()) {
		   $.ajax({ 
			   type: 'POST',
			   url:'../testdubbo/insertUser', 
			   data:{name:name,code:code,sex:sex,state:state}, 
   ...
   function editData(row){
	   $('#editvue').on('show.bs.modal', 	// 给模态框赋值
			function (event) { 	
				   ...
				   $('#modal-code').val(row.usercode);
				   $('#modal-sex').selectpicker('val',row.usersex);
				   $('#modal-state').selectpicker('val',row.userstate);
   ...
   // 保存编辑
   function editUser111(){
	   
	   var id=$('#modal-id').val();
	   var name=$('#modal-name').val();
	   var code=$('#modal-code').val();
	   var sex=$('#modal-sex').selectpicker('val');
	   var state=$('#modal-state').selectpicker('val');
	   ...
	   if (bv.isValid()) {
		   ...
		   $.ajax({ 
			   type: 'POST',
			   url:'../testdubbo/editUser', 
			   data:{id:id,name:name,code:code,sex:sex,state:state}, 
			   cache:false,// false是不缓存，true为缓存
			   async:true,// true为异步，false为同步
			   dataType:"json",
			   beforeSend:function(){ // 请求前
